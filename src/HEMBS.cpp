...
void HEMBS::forward_bitsets_insert_IPv4(const rule* r)
{
	...
	{                                                                       // 0,1,2,3 attribute  i means the (i+1)-th byte from high/right end
		bitsets[ai][(uint32_t)r->source_ip[3 - ai]][unitNo] |= offsetMask; // �����˴μ������ڶ�����ʱ�ô�˴洢�ɱ��⣻�����õ�λ�ֽ�Ϊ0������
	}

	if (mask_by8 < 4)
	{
		uint32_t rightShiftBits = 8 - ((uint32_t)r->source_mask & 7); // �ٽ��ֽڣ��Ӹ�λ���mask_by8+1���ֽڣ��ϵ�����λ��
		uint32_t lowValue =
			((uint32_t)r->source_ip[3 - mask_by8] >> rightShiftBits) << rightShiftBits; // Bug1: ���� ��3-����
	...
	if (mask_by8 < 4)
	{
		...
		for (uint32_t bi = lowValue; bi < lowValue + (1 << freeBitsNum); bi++)
			bitsets[4 + mask_by8][bi][unitNo] |= offsetMask; // Bug2: �ټ���4��
...
// & b[5][bi] & b[6][bi] & b[7][bi] & b[8][bi] & b[9][bi] & b[10][bi];
		...
		//    __builtin_ctzl ��λ�жϰ汾 
		// 		while (andResult)
		// 		{
		// #if DEBUG
		// 			checkNum++;
		// #endif
		// 			matchRuleNo= (bi<<6)+ __builtin_ctzl(andResult); // �� andResult �д�������һ�� 1 �ұߣ��ĸ���
...
// 			}
			...
			{ // 949us
				...
				checkNum++;
#endif
				matchRuleNo = (j << 6) + __builtin_ctzl(orResult); // �� andResult �д�������һ�� 1 �ұߣ��ĸ���
				if (rules[matchRuleNo].source_port[0] <= msg->source_port &&
					msg->source_port <= rules[matchRuleNo].source_port[1] &&
					rules[matchRuleNo].destination_port[0] <= msg->destination_port &&
					msg->destination_port <= rules[matchRuleNo].destination_port[1])
...
//		}
		...
		{ // ��ʼ���һ��λ��
			...
			for (uint32_t bi = abi << log2AggRatio; bi < (abi << log2AggRatio) + aggRatio; // ��ʱ�������λ�Ŵ���64��һ��λ��Ԫ������һ������Ԫ����
				 bi++) // ��abi��64����λ����aggRatio��64һ��λ
			{ // �� bi �� 64 bits of bitsets

				// aggRatio<=64�����
				// 1.0 ������
				// bool flag = false;
				// for (uint32_t j = 0; j < numBitsTo64; j++) // $numBitsTo64 ������λ���� 64 ��һ��λ
				// {
				// 	if (aggAndResult & 1)
				// 	{
...
#else // ��ʱһ������λ����1������64һ��λ��Ԫ��ÿ�ּ�� $num64Bits ��һ����Ԫ

//			for (uint32_t ari = abi << log2AggRatio; ari < (abi << log2AggRatio)+aggRatio; ari+=num64Bits) // $numBitsTo64 ������λ���� 64 ��һ��λ
//				 {
//				 	if (aggAndResult & 1)
//				 	{
			while (aggAndResult) // �������for��ȣ��Ƚϴ����� 284 �½��� 138
			{
#if DEBUG
				cmpNum += 1;
#endif
			...
			while(aggOrResult){
				...
				for (uint32_t bi = ari; bi < ari + num64Bits; bi++)
				{
					...
					{ 
						...
						checkNum++;
#endif
						matchRuleNo = (bi << 6) + __builtin_ctzl(orResult); // �� andResult �д�������һ�� 1 �ұߣ��ĸ���
						if (rules[matchRuleNo].source_port[0] <= msg->source_port &&
							msg->source_port <= rules[matchRuleNo].source_port[1] &&
							rules[matchRuleNo].destination_port[0] <= msg->destination_port &&
							msg->destination_port <= rules[matchRuleNo].destination_port[1])
...
void HEMBS::RLE_forward_init_bitsets_IPv4(uint32_t numRule)
{
	...
	rleCode[8].resize(NUM_PROTOCOL + 1); // Bug: ��+��1
#endif
...
#if DEBUG
		cmpNum += HEM_BS_NUM_ATTR * 3 + 1; // �����for, ����for�������if, �Լ�����if�ж�
...
{
	uint32_t n64 = aggRatio; // ���� n64 �� 64λΪ0
	...
	for (int ai = 0; ai < HEM_BS_NUM_ATTR; ai++)
	{
		...
		for (uint32_t bi = 0; bi < numv; bi++)
		{
			...
			{
				uint32_t t = k;
				while (t < min(numUnit, k + n64) && bitsets[ai][bi][t] == 0) // ��ʵ���Բ���AFBS��ֱ���ж� aggBitsets
					t++;
